...
class MedicalRAG:
    ...
    def ingest_file(self, document_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # الخطوة 2: تلخيص الصور (Azure GPT-4o)
            ...
            # الخطوة 4: التقسيم (هنا سيتم استدعاء التعديل الجديد المحلي)
            self.logger.info("4. Chunking document into sections (Local Recursive Splitting)...")
            document_chunks = self.content_processor.chunk_document(formatted_document)
            
            # إضافة تحسين: التأكد من وجود chunks قبل الانتقال لـ Qdrant
            if not document_chunks:
                raise ValueError("No chunks were generated from the document.")
                
            self.logger.info(f"   Document split into {len(document_chunks)} chunks successfully")

            # الخطوة 5: التخزين في Qdrant
            self.logger.info("5. Creating vector store knowledge base...")
            # تأكد أن الـ VectorStore تم تحديث مكتباته كما فعلنا سابقاً
            ...
            # تحسين: تسجيل الخطأ بشكل تفصيلي للمساعدة في تتبع مشاكل الـ VectorStore
            self.logger.error(f"❌ Error during ingestion of {document_path}: {e}")
            import traceback

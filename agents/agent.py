...
"""
Agent  System for Multi-Agent Medical Chatbot
...
def create_agent_graph():
    ...
    def run_brain_tumor_agent(state: AgentState) -> AgentState:
        ...
        diagnosis = AgentConfig.image_analyzer.brain_tumor_agent.predict(image_path)
        
        response_text = f"The analysis of the brain MRI suggests: **{diagnosis}**."
    ...
    def run_skin_lesion_agent(state: AgentState) -> AgentState:
        """Handle skin lesion image analysis using the Swin Transformer classifier."""
        print(f"Selected agent: SKIN_LESION_AGENT")
        
        current_input = state["current_input"]
        image_path = current_input.get("image", None)

        predicted_class, confidence = AgentConfig.image_analyzer.skin_lesion_agent.predict(image_path)
    
        if predicted_class:
            response_text = (
                f"Based on the Swin Transformer analysis, the suspected diagnosis is "
                f"**{predicted_class.upper()}** (Confidence: **{confidence:.2%}**)."
            )
            response = AIMessage(content=response_text)
        else:
            response = AIMessage(content="The skin lesion analysis failed. Please ensure the image is clear.")
...
# Initialize the graph ONCE (global scope)
medical_graph = create_agent_graph()


def process_query(query: Union[str, Dict], chat_history: Optional[List] = None) -> Dict:
    """
    Process a user query through the compiled LangGraph.
    
    Args:
        query: User query (string or dict with 'text' and 'image')
        chat_history: Optional conversation history (not used with MemorySaver)
    ...
    """
    # Extract text for the thread history
    if isinstance(query, dict):
        user_text = query.get("text", "") 
        image_path = query.get("image", None)
        input_content = f"{user_text}\n[SYSTEM: User uploaded an image at {image_path}]"
    else:
        input_content = query
    ...
    result = medical_graph.invoke(initial_state, thread_config)

    # Return the FULL result dict (not just string)
    return result
